...
export class ToneKeyboard extends LitElement {
	...
	getNoteByTouchId(id): ToneKeyboardNote | undefined {
		const octaves = Array.from(this.shadowRoot.querySelectorAll("tone-keyboard-octave")) as ToneKeyboardOctave[];
	...
	private _touchmove(event: TouchEvent) {
		Array.from(event.changedTouches).forEach(e => {
			...
			if (element && element.shadowRoot) {
				const note = element.shadowRoot.elementFromPoint(e.clientX, e.clientY) as ToneKeyboardNote | undefined;
	...
	noteon(midi) {
		...
		const toneOctave = this.shadowRoot.querySelector(`tone-keyboard-octave[octave="${octaveNumber}"]`) as ToneKeyboardOctave;
		if (toneOctave) {
