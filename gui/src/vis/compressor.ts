...
customElements.define("tone-compressor-vis", class extends VisBase {
	...
	protected async generate() {
		...
		const values = retArray.map((_, i) => {
			...
			// apply the ratio
			if (i < kneeBegin) { // linear
				return linearVal;			
			} else if (kneeBegin <= i && i <= kneeEnd) { // in the knee
	...
	loop() {
		...
		(this.shadowRoot.querySelector("#reduction #fill") as HTMLElement).style.height = `${reduction.toFixed(1)}%`;
	}
