...
customElements.define("tone-number", class extends ToneValue<number> {
	...
	firstUpdated(changed) {
		...
		window.addEventListener("mousemove", (e: MouseEvent) => {
			if (this.dragging) {
				...
				const multiplier = this.shift ? this.alt ? 0.01 : 10 : this.alt ? 0.1 : 1;
				this.value = this.dragstartValue + Math.floor(yDiff / 10) * multiplier;
		...
		return html`
			<div id="container">
				...
				<input 
					...
					name="number" type="text" .value="${this.beautifyValue(this.value)}">
			</div>
			`;
	}
});
