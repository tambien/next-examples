	...
	private renderValue(name: string, values: any, path: string[]) {
		...
		if (typeof value === "number") {
			...
			return html`
				<details 
					class="sub-value"
					@toggle=${(e) => e.target.querySelector("mwc-icon").textContent = e.target.open ? "arrow_drop_down" : "arrow_right"}>
					<summary>
						<mwc-icon>
							arrow_right
						</mwc-icon>
...
export type ElementOptions = ToneAudioNodeElementOptions & {parent?: HTMLElement};
