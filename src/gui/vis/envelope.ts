...
customElements.define("tone-envelope-vis", class extends VisBase {
	...
	render() {
		...
		if (this.tone) {
			...
			const totalDuration = 1.1 * (this.tone.toSeconds(this.tone.attack) + this.tone.toSeconds(this.tone.decay) + this.tone.toSeconds(this.tone.release));
			...
			const rounding = subdivision.toString().split(".")[1] && subdivision.toString().split(".")[1].length || 0;
	
			for (let time = 0; time < totalDuration; time += subdivision) {
		...
		return html`
			...
			${super.render()}
			<div id="timeline">
				${times.map(({ x, time }) => html`
					<span style="left: ${(x * 100).toFixed(2)}%">
						${time}s
					</span>
				`)}
			</div>
